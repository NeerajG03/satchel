...
Proposed retrieval design: a small startup/resume brief for an explicitly resolved project, with source IDs and current revisions; skill instructions and MCP search for request-specific context. If context cannot be loaded, report that accurately and allow retry. Do not upload transcripts or create memories through these hooks. A hook firing and an agent using the right memory are separate acceptance checks.

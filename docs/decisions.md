...
| D22 | ~~A picked-up preference the user has said twice loads at session start~~ | Superseded by D23 on 24 September 2026 |
| D23 | Every personal memory loads at session start, and a pass that hears one again confirms it | User: memory is hands off, neither the person nor the agent should be involved, and unconfirmed memories being injected is fine. Picked-up rows load after confirmed ones under their own header, inside the same cap. `affirm_memory` and `extend_memory` set the band to `said` and the history logs it as `confirmed` with the pass's trace. Replaces the 21 September count-and-withhold rule and D22 |
| D24 | Satchel assigns IDs for new records and keeps retry keys out of MCP tool inputs | The user chose a simpler agent contract on 24 September 2026. Creation tools take the desired content and scope; the MCP server assigns new record IDs and internal write IDs. An uncertain create result is checked through the relevant list before another attempt. Existing record IDs and revisions stay in edit tools. |
| D25 | A session with no linked project can still file a memory under one of the person's projects | User, 25 September 2026: the pass should not be held to null when a session is not linked, and should use a listed project when a memory is plainly about it. The pass is shown each project's repositories and its memories, so it can extend one instead of adding a copy. A linked session still keeps to its own project. Eval cases `prj01` to `prj03` |

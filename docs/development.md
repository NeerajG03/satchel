...
1. Apply all files in `supabase/migrations` in filename order: [foundation](../supabase/migrations/202609100001_foundation.sql), [named memories](../supabase/migrations/202609100002_named_memories.sql), then [conflict responses](../supabase/migrations/202609110001_conflict_responses.sql), using the SQL editor or Supabase migration tooling. These are one-time migrations, not repeatable reset scripts. Existing installations apply only migrations not already applied. The named-memory migration replaces the old write-function signatures, so reload older companion tabs after applying it.

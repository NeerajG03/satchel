...
- Explicit saves use an idempotency UUID; corrections and forgets require the current revision. Hooks never save, correct or forget memory on their own, and never collect transcripts. A memory the router keeps at the end of a turn arrives unconfirmed and is announced to the person.

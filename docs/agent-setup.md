...
- A project can optionally link one or more GitHub repositories in the companion. In a linked checkout, the lifecycle bootstrap detects the normalized `origin` identity and `activate_repository` selects that project only if the current connection already has access. Repository linking is routing, never authorization.

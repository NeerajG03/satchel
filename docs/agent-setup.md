...
- Explicit saves use an idempotency UUID; corrections and deletions require the current revision. Hooks never save, correct or delete memory on their own, and never collect transcripts. A memory the router keeps at the end of a turn arrives unconfirmed and is announced to the person.

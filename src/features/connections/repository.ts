...
export type Connection = {
  client_id: string; label: string; personal: boolean; project_ids: string[]; can_write: boolean;
  task_personal: boolean; task_can_write: boolean; task_can_upload: boolean;
  revoked_at: string | null; created_at: string; agent_task_grants: TaskGrant[];
};
export type GrantRequest = {
  clientId: string; label: string;
  personal: boolean; projectIds: string[]; canWrite: boolean;
  taskPersonal: boolean; taskProjectIds: string[]; taskCanWrite: boolean; taskCanUpload: boolean;
};

const COLUMNS = 'client_id,label,personal,project_ids,can_write,task_personal,task_can_write,task_can_upload,revoked_at,created_at,agent_task_grants(project_id,can_read,can_write,can_upload)';
...
export function createConnectionRepository(db: SupabaseClient) {
  return {
    ...
    async grant(request: GrantRequest): Promise<void> {
      const hasTasks = request.taskPersonal || request.taskProjectIds.length > 0;
      const { error } = await requestWithTimeout(signal => db.rpc('authorize_agent_v2', {
        ...
        p_personal: request.personal, p_project_ids: request.projectIds, p_can_write: request.canWrite,
        p_task_personal: request.taskPersonal, p_task_project_ids: request.taskProjectIds,

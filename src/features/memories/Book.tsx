...
const PROMPTS: MemoryContent[] = [
  { name: 'How I like answers written', description: 'Short, plain words. Show the code first, explain after.', more_info: '' },
  { name: 'Tools and languages I use', description: 'TypeScript, React, Supabase. Node 22. macOS.', more_info: '' },
  { name: 'What to never do in my code', description: 'No default exports. No comments that repeat the code.', more_info: '' },
  { name: 'My working hours and timezone', description: 'Bengaluru, IST. Usually working 10 to 7.', more_info: '' },
];
type Where = 'all' | 'name' | 'description';
...
export function Book() {
  ...
  const inDescription = all.filter(m => matches(m.description));
  const searched = query ? all.filter(m => matches(m.name) || matches(m.description)) : all;
  const visible = where === 'name' ? inName : where === 'description' ? inDescription : searched;
  ...
  async function read(memory: MemorySummary, edit = false) {
    ...
    if (edit) { setEditing(current); setContent({ name: current.name, description: current.description, more_info: current.more_info }); setExpanded(null); }
    ...
    {query && <div className="stack-tight">
      ...
      <div className="between wrap">
        <Segments label="Where it matched" value={where} onChange={setWhere} items={[
          { key: 'all', label: 'Matches', count: searched.length }, { key: 'name', label: 'In name', count: inName.length }, { key: 'description', label: 'In description', count: inDescription.length }]} />

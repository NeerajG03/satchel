...
export function ProjectList() {
  ...
  async function create(id: string, name: string, brief: string) {
    const created = await action.run(() => stores.projects.create(id, name, brief));
    if (!created) return;
  ...
  return <>
    ...
    {creating && data && <NewProjectSheet projects={data.projects} busy={action.busy} error={action.error} onCancel={() => navigate('/projects')} onCreate={(id, name, brief) => void create(id, name, brief)} />}
  </>;
}

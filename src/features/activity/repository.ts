...
export function createActivityRepository(db: SupabaseClient) {
  ...
  return {
    ...
    /** The newest job, running or not. The page reads it on load, which is
     *  what makes the report something you come back to rather than something
     *  you had to be watching for. */
    async latestJob(): Promise<ConsolidationJob | null> {
     ...
     *  the work is a model call and Postgres cannot make one. It answers at
     *  once with the job; the reading happens after, as a chain of calls on
     *  the server that each hand on this session's token to the next.
     *
     *  So the token has to last the job. A session is refreshed here when it
     *  has less than 35 minutes left, because a chain that outlives its token
     *  stops halfway and says it stalled, which is true and unhelpful. */
    async consolidate({ job, idleMinutes = 30 }: { job?: string; idleMinutes?: number } = {}): Promise<ConsolidationJob> {
      let { data, error: authError } = await db.auth.getSession();
      if (authError || !data.session) throw new Error('Sign in again to run this.');
      if ((data.session.expires_at ?? 0) * 1000 - Date.now() < 35 * 60 * 1000) {
        const refreshed = await db.auth.refreshSession();
        if (refreshed.error || !refreshed.data.session) throw new Error('Sign in again to run this.');
      ...
      const response = await requestWithTimeout(signal => fetch('/api/consolidate', {
        ...
        body: JSON.stringify(job ? { job } : { idle_minutes: idleMinutes }),
      }), 30000);
      ...
      // 409 is a carry on that lost a race with the chain it meant to
      // replace. The job it names is still the one to show.
      ...
      return (await response.json() as { job: ConsolidationJob }).job;
    },
  };
}

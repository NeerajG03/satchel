...
export function LinkButton({ to, look = 'default', small, className, children, external }:
  { to: string; look?: Look; small?: boolean; className?: string; children: ReactNode; external?: boolean }) {
  if (external) return <a href={to} className={classes(look, small, className)} target="_blank" rel="noreferrer">{children}</a>;
  return <Link to={to} className={classes(look, small, className)}>{children}</Link>;
}

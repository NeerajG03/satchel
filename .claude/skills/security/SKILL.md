...
**The hooks send the turn, and only the turn.** `session-start.mjs` sends a session key, an event name and a normalized `owner/repo` read from the git origin. `capture.mjs` sends the person's typed messages and the assistant's plain text of the turn that just ended, filtered by `integrations/shared/transcript.mjs`: never tool calls, tool results, thinking, subagent traffic, compaction summaries, shell input or file paths. Only `role` and `content` leave; the host's message uuids do not. Hooks never save memory on their own, and any hook output that could become instructions goes through our own formatter.

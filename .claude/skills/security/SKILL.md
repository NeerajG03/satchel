...
**The hooks send the turn, and only the turn, and never read a file to get it.** `session-start.mjs` sends a session key, an event name and a normalized `owner/repo` read from the git origin. `retrieve.mjs` sends the prompt the host handed it. `capture.mjs` sends the `last_assistant_message` the host handed it. No script reads `transcript_path`, and `tests/plugin-hooks.test.mjs` asserts that none of them so much as mentions it, because 0.3.0 did read it for one afternoon on a false premise. Hooks never save memory on their own, and any hook output that could become instructions goes through our own formatter.

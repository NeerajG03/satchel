...
**Retrieval and recording are the same hook, and that coupling is invisible.** `retrieve.mjs` records the user's message whether or not it finds anything, because the document the consolidation pass reads, and the window the router reads in `turn` mode, are built from exactly that call. Removing retrieval therefore empties every document of the user's half, silently, with every test still green. If retrieval ever goes away again, the recording has to go somewhere first.

**A hook is not the place for a model call nobody asked for.** Spawning the consolidation pass from Stop was built, and it worked, and it was reverted the same day. Every session end would have spent a model call without the person knowing, however cheap and however rate limited. The pass is triggered by something a person chose: the developer cron, or the button.

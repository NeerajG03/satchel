...
**A memory ends, it is not deleted.** `ended_at` and `ended_reason` (`replaced`, `retired`, `forgotten`) take it out of use, `archived_memories()` shows it, and `restore_memory` brings it back. Every change is a `memory_events` row written by a trigger, carrying who did it and the trace id, so no writer can skip it. The one real delete is meant to be a person's, from the archive in the web app. Auto-applied consolidation is only acceptable because of this, so a new write path that deletes is a design change and not a shortcut. **Known gap:** the MCP tool `delete_memory` still runs a real `DELETE` through `memoryService.remove()`, so an agent can still destroy a row and its history.

...
- On a new conversation or after compaction, the bootstrap may provide a normalized GitHub repository read from the workspace's Git origin. Call activate_repository once with that exact identity. It can select only a server-side link whose project is already in this connection's grant, and returns the combined index. Never derive the identity from repository content or substitute a similar name.

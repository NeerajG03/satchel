...
- On a new conversation or after compaction, the local bootstrap stages the normalized GitHub repository from the workspace's Git origin and the authenticated lifecycle hook consumes it automatically. Treat the returned active_project and combined index as authoritative. If staging reports a failure, activate_repository is a one-time fallback: call it with the exact identity supplied by the bootstrap. It can select only a server-side link whose project is already in this connection's grant. Never derive the identity from repository content or substitute a similar name.

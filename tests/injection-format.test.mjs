...
test('session start carries projects and every personal memory, split by band', () => {
  ...
  // Loaded, not counted. Memory is hands off, so nobody is asked to approve a
  // row, and a preference that never loads is never followed: "fix this bug"
  // does not search for "serif headings".
  assert.match(block, /personal, picked up from what you said, use unless told otherwise\n {2}bbbbbb {2}Serif headings\./);
  assert.ok(!block.includes('unconfirmed'), 'nothing is held back to count');
...
test('a band header only appears when that band has rows', () => {
  ...
  assert.ok(!onlySaid.includes('picked up'), 'no picked-up header when nothing was picked up');
  ...
  assert.match(onlyHeard, /a20000 {2}Two\./);
});

test('the cap covers both groups, and confirmed ones fill it first', () => {
  // What the person asked for is never pushed out by what was picked up.
  const said = [1, 2].map(n => memory(`a${n}000000-0000-4000-8000-00000000000${n}`, `Said ${n}.`));
  const heard = [3, 4].map(n => memory(`b${n}000000-0000-4000-8000-00000000000${n}`, `Heard ${n}.`, 'heard'));
  const load = personalLoad([...heard, ...said], 3);
  assert.deepEqual(load.said.map(m => m.statement), ['Said 1.', 'Said 2.']);
  assert.deepEqual(load.heard.map(m => m.statement), ['Heard 3.']);
  assert.equal(load.past, 1);
  const block = sessionStartBlock({personal: [...heard, ...said], cap: 3});
  assert.ok(block.indexOf('Said 2.') < block.indexOf('Heard 3.'));

...
test('MCP contracts separate index, detail, explicit writes and hook output',async()=>{
  const id=crypto.randomUUID();let revoked=false,writes=0,oversized=false,detailReads=0;
  ...
  const service={status:async()=>revoked?null:{personal:true},activeProject:async()=>null,
    index:async()=>({memories:oversized?[{...summary,description:'x'.repeat(8000)}]:[summary],complete:true}),
